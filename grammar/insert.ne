...
insert_statement -> kw_insert %ws collection_name %ws insert_collection_columns {% d => {
    return{
        "type":"insert",
        "params":{
            "document" : d[4],
...
bulk_insert_statement -> kw_bulk %ws kw_insert %ws collection_name %ws column_object_array {% d => {
    return{
        "type":"bulk insert",
        "params":{
            "documents" : d[6],
...
insert_collection_columns -> column_name_array {% d => d[0] %}
insert_collection_columns -> column_object {% d=> d[0] %}
